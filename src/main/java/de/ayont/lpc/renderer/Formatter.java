...
import static de.ayont.lpc.renderer.LPCChatRenderer.LEGACY_CHAR_CODE;
...
public enum Formatter {
    ...
    LEGACY(
        "legacy",
        (line) -> getLegacySerializer().deserialize(convertLegacyHex(line.replace(LEGACY_CHAR_CODE, '&')))
    ),
    SMART(
        "smart",
        (line) -> {
            // First replace any legacy chars with &
            String mutableLine = convertLegacyHex(line.replace(LEGACY_CHAR_CODE, '&'));
    ...
    Formatter(@NotNull String identifier, @NotNull Function<String, Component> formatter) {
    ...
    public static @NotNull Optional<Formatter> getById(@NotNull String identifier) {
